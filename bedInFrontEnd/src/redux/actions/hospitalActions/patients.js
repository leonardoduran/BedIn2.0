...
export function fetchGetPatients() {
  return dispatch => {
    ...
    return fetch('./hospital/patientRequest', {
      ...
      .then(patients => {
        if (patients.error) alert(patients.error);
        else dispatch(getPatients(patients));
      })
  ...
  return dispatch => {
    ...
    return fetch('./hospital/patientRequest/reasonsReject', {
      ...
      .then(reasons => {
        ...
        else dispatch(getReasons(reasons));
      })
      .catch(err => dispatch(failedToFetch(err)));
  };
}
    ...
    return fetch(`./hospital/patientRequest/${state}`, {
      ...
      .then(patients => {
        if (patients.error) alert(patients.error);
          ...
          switch (state) {
            case 'Aceptado':
              dispatch(getAcceptedPatients(patients));
              break;
            case 'Rechazado':
              dispatch(getRejectedPatients(patients));
              break;
            default:
...
export function fetchSetAllViewed(patients) {
  return dispatch => {
    ...
    return (
      fetch('./hospital/patientRequest/allViewed', {
        method: 'PUT',
        credentials: 'include',
        headers: {
          Accept: 'application/json',
